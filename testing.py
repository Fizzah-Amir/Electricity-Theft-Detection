...
labels = ["Correctly Normal (TN)", "Correctly Caught Theft (TP)",
          "Missed Theft (FN)", "False Alarm (FP)"]
sizes  = [correct_normal, correct_theft, missed_theft, false_alarm]
colors = ["#42A5F5", "#66BB6A", "#EF5350", "#FFA726"]
total  = sum(sizes)

# ── LEFT: Pie chart (clean, no labels on slices) ──
ax1 = axes[0]
wedges, autotexts = ax1.pie(
    sizes,
    colors=colors,
    startangle=140,
...
ax1.set_title("Distribution (%)", fontsize=12, fontweight="bold", pad=12)
ax2 = axes[1]
short_labels = ["Correctly\nNormal (TN)", "Correctly Caught\nTheft (TP)",
                "Missed\nTheft (FN)", "False\nAlarm (FP)"]
bars = ax2.barh(short_labels, sizes, color=colors,
                edgecolor="white", linewidth=1.5, height=0.5)
for bar, val in zip(bars, sizes):
    pct = val / total * 100
    ax2.text(bar.get_width() + max(sizes) * 0.01,
             bar.get_y() + bar.get_height() / 2,
             f"{val}  ({pct:.1f}%)",
             va="center", fontsize=11, fontweight="bold")
ax2.set_xlim(0, max(sizes) * 1.3)
ax2.set_xlabel("Number of Windows", fontsize=11)
ax2.set_title("Counts & Percentages", fontsize=12, fontweight="bold", pad=12)
ax2.spines["top"].set_visible(False)
ax2.spines["right"].set_visible(False)
ax2.tick_params(axis="y", labelsize=10)

fig.suptitle("Prediction Breakdown — GB Scratch on Test Data",
             fontsize=14, fontweight="bold", y=1.01)
plt.tight_layout()
plt.savefig("gb_scratch_testing/graph2_prediction_breakdown.png",
...
print("=" * 50)
print("""
  Files saved in gb_scratch_testing/ folder:

  DATA FILES:
    all_predictions.csv       — every window prediction
    suspicious_users.csv      — flagged theft windows
    alert_report.txt          — full alert report

  GRAPHS:
    graph1_probability_distribution.png
    graph2_prediction_breakdown.png
    graph3_risk_levels.png
